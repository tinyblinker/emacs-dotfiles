:PROPERTIES:
:ID:       f616c8db-9123-49d3-83c8-1e967162acae
:END:
#+title: 在emacs中尝试使用EAF框架的记录
#+date: 2026-08-26 18:13
#+filetags:

* 背景
早上去咖啡馆,突然想起来折腾一下EAF框架,看emacs直接调用浏览器的体验,结果可谓是很抽象,
其实本来打算好好写那个自制的cava的,可是忍不住无聊折腾的手....Orz,下次一定
* 过程
** 项目介绍
- what is EAF(emacs-application-framework)?
...
- 项目链接:[[https://github.com/emacs-eaf/emacs-application-framework][github地址]]
** 尝试use-packge的:vc安装EAF
本来想直接用use-package的:vc选项安装,结果却是发现越想越复杂,浅克隆根本不支持,自己也不想修改太多(其实是不会修改)
emacs的原生use-package的行为,所以整体了一个圈后就又绕回来了,所以还是得按照README上的指导配置来做,别不听劝
** 按照README来做
1.先克隆仓库到指定文件夹
#+begin_src bash
...
#+end_src
5.Customize EAF according to the github official wiki
** 糟糕的体验
#+begin_src bash
cd emacs-application-framework
chmod +x ./install-eaf.py
...
#+begin_src python
  ...
  if __name__ == '__main__':
      main()
#+end_src
评价是npm全局安装了一些依赖,加上aur安装了一些依赖,还有一些archlinux系统包依赖,虽然这实现外部调用代码,但是这
实在是让系统的环境陷入了混乱,实在是过于重,个人倾向于尽可能零依赖和利用内置功能,所以经过今天的尝试,确认实在是不
喜欢这种形式的emacs插件,我应该会保持用内置功能和经典简约插件和社区经典理念下的插件并自行实现微型插件;
* 总结
过于纠结,应该优先实现cava的本来,因为自己某种逃避???又在过度折腾,这下又回来了一个圈,
应当优先保证生产力!!!!
