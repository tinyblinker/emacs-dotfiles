...
;; set china mirror srcs of repos
...
;; if `package-refresh-contents' needs to be run during startup.
(require 'time-date) ;; add "time-date" lib

(defvar crafted-package-perform-stale-archive-check t ;; as doc strings said
  "Check if any package archives are stale.

Set this value in your `early-init.el' file.")

(defvar crafted-package-update-days 1 ;; as doc strings said
...
archive found or nil if they are all up-to-date"
  (interactive) ;; can be invoked by M-x
